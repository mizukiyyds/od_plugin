...
//ollydbg bug1:  OD的long double类型为10字节，但是vs编译器里是8字节
//ollydbg bug2:  所有函数前面需要加一个下划线，因为导出函数命名原因
//ollydbg bug3:  这个头文件使用了pack预编译指令，但是结尾却没有用pack()取消，害得我折腾了几个星期看了好久才发现bug
#pragma pack(1)
struct float80
{
    double v;
...
extc void    cdecl _Addtolist(long addr,int highlight,char const* format,...);
extc void    cdecl _Updatelist(void);
extc HWND    cdecl _Createlistwindow(void);
extc void    cdecl _Error(char const* format,...);
extc void    cdecl _Message(ulong addr,char const* format,...);
extc void    cdecl _Infoline(char const* format,...);
extc void    cdecl _Progress(int promille,char const* format,...);
extc void    cdecl _Flash(char const* format,...);
...
extc int     cdecl _Printfloat10(char *s,float80 ext);
...
extc int     cdecl _Getlong(char const* title,ulong *data,int datasize,
               char letter,int mode);
extc int     cdecl _Getlongxy(char const* title,ulong *data,int datasize,
               char letter,int mode,int x,int y);
extc int     cdecl _Getregxy(char const* title,ulong *data,char letter,int x,int y);
extc int     cdecl _Getline(char const* title,ulong *data);
extc int     cdecl _Getlinexy(char const* title,ulong *data,int x,int y);
extc int     cdecl _Getfloat10(char const* title,float80 *fdata,
               uchar *tag,char letter,int mode);
extc int     cdecl _Getfloat10xy(char const* title,float80 *fdata,
               char *tag,char letter,int mode,int x,int y);
extc int     cdecl _Getfloat(char const* title,void *fdata,int size,
               char letter,int mode);
extc int     cdecl _Getfloatxy(char const* title,void *fdata,int size,
               char letter,int mode,int x,int y);
...
extc int     cdecl _Gettext(char const* title,char *text,
               char letter,int type,int fontindex);
extc int     cdecl _Gettextxy(char const* title,char *text,char letter,
               int type,int fontindex,int x,int y);
extc int     cdecl _Gethexstring(char const* title,t_hexstr *hs,
               int mode,int fontindex,char letter);
extc int     cdecl _Gethexstringxy(char const* title,t_hexstr *hs,int mode,
               int fontindex,char letter,int x,int y);
extc int     cdecl _Getmmx(char const* title,uchar *data,int mode);
extc int     cdecl _Getmmxxy(char const* title,char *data,int mode,int x,int y);
extc int     cdecl _Get3dnow(char const* title,uchar *data,int mode);
extc int     cdecl _Get3dnowxy(char const* title,char *data,int mode,int x,int y);
extc int     cdecl _Browsefilename(char const* title,char *name,char *defext,
               int getarguments);
...
#define   DEC_FLOAT10  0x09            // Accessed as float80
...
typedef struct t_reg {                 // Excerpt from context
  ...
  float80    f[8];                 // Float registers, f[top] - top of stack 
...
typedef struct t_result {              // Result of expression's evaluation
  ...
  union {
    ...
    float80  f; };                 // Value as 80-bit float
  union {
...
extc int     cdecl _Findreferences(ulong base,ulong size,ulong addr0,ulong addr1,
               ulong origin,int recurseonjump,char const* title);
extc int     cdecl _Findstrings(ulong base,ulong size,ulong origin,char const* title);
extc int     cdecl _Findalldllcalls(t_dump *pd,ulong origin,char const* title);
extc int     cdecl _Findallcommands(t_dump *pd,t_asmmodel *model,
               ulong origin,char const* title);
